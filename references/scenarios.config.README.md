...
| `writeGuardAllowlist` | array of strings | EXTRA absolute write-roots the subagent write-guard should permit, beyond the always-allowed `${CLAUDE_PROJECT_DIR}` + `/tmp` + `/private/tmp` + `/var/folders/*` + `/private/var/folders/*` (both macOS spellings — the guard matches after `realpath -m`, which resolves the symlinks). Leave `[]` unless scenarios legitimately write outside the project (e.g. a fixtures tree under `$HOME/…`). Each entry is matched as a prefix. |

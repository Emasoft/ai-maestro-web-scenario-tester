...
turns × per-turn-context** — the whole transcript is re-read every turn, so a
careless run balloons past 100M tokens). The eight load-bearing techniques —
turn batching, fixed-first load order, scoped reads, errors-only tool output,
clipped screenshots, dev-browser-only verification, errors-only logs, and
concise DRY reports — live in
`${CLAUDE_PLUGIN_ROOT}/references/token-discipline.md`. Read that reference
before any run; the `amwst-scenario-runner` enforces the techniques per step.
...
<example>
user: Write a scenario for the login flow, then run it.
assistant: [loads amwst-create-scenario, authors SCEN-025_login-flow.scen.md, validates it with amwst-validate-scenario, then loads amwst-run-scenario which runs the runner and the proposer]
Authored + validated SCEN-025. Run verdict: PASS (18/18 steps).
Report: reports/scenarios-runner/SCEN-025_20260708T160000Z.report.md
</example>

<example>
user: Run scenarios 5-12 overnight.
assistant: [loads amwst-run-scenarios-batch — the state-machine-driven batch dispatches one runner + one proposer per scenario, unattended]
Batch armed for SCEN-005..SCEN-012; state file + heartbeat recovery in place.
</example>
